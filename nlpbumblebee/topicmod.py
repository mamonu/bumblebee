...
class Tuples2ColumnsTransformer(BaseEstimator, TransformerMixin):
    """
    Class for building sklearn Pipeline step. 
    This class turns columns from a pandas.Dataframe (type Series) that
    contain tuples of integers or floats into separate array columns, one column
    for each element of the tuple, so that can be used as input in ML pipelines.
...
class LdaRankedTopicsTransformer(BaseEstimator, TransformerMixin):
    """
    This class returns an array of the indeces of topics generated by sklearn_api.ldamodel.LdaTransformer 
    in their ranked order, and the corresponding rank-ordered probabilities
    
    [.20, .10, .70] => array([2., 0., 1., 0.7, 0.2, 0.1)
    ...
    def transform(self, XArray):        #XArray: array of topic probabilities lists
        tindex_result = [(np.argsort(np.array(alist))[::-1]) for alist in XArray]
        pbs_result = [(np.sort(np.array(alist))[::-1]) for alist in XArray]
    
        return np.array(np.concatenate((tindex_result, pbs_result), axis=1))